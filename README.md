...
「视图 → 背景设置…」可以选一张图片作为主窗口背景，再用三个滑块和一项配色选择调整观感。

**图片铺满整个窗口。** 图片以 `background-size: cover` + `background-attachment: fixed` 画在页面 `body` 上，所以无论窗口什么比例都不会变形、页面内容滚动时也不会跟着跑。

**界面半透明，文字不透明。** 滑块调节的是界面面板的不透明度：调低时侧边栏、卡片、输入框变半透明，背景图从后面透出来。实现方式是覆盖 DSH 主题自己的背景设计令牌，而不是给整个界面加 `opacity`——后者会把文字和图标一起冲淡，看起来发灰。

三个滑块各有分工：**面板不透明度**（侧边栏、卡片、输入框，最低 20%）、**画布不透明度**（主对话区的底色，可以调到 0，让背景图完整露出来）、**背景压暗**（在图片上叠一层可调的黑色，图片太亮时调高）。

**界面配色跟着背景图走。** 浅色半透明界面压在深色壁纸上是一层发灰的牛奶膜，深色文字压在亮图上又认不出来——这正是「文字跟背景色不搭」的来源。配色选「跟随背景图」时，外壳量一下图片的平均亮度（`nativeImage` 解到 32px 宽再求均值），偏暗就换成深色界面。

做法不是改 DSH 的主题设置，而是把 DSH 自己的两套调色板从页面里读回来（`document.styleSheets` 里 `body` 与 `body[data-ds-dark-theme]` 两条规则的全部 `--dsw-*` 声明），再把需要的那一套以 `!important` 重新声明在 `body` 上。DSH 写在元素内联样式里的令牌也压不过 `!important`，所以即使用户把 DSH 主题固定成浅色，界面照样按背景图渲染色。外壳同时同步 `nativeTheme.themeSource`，让窗口边框、菜单和原生控件保持一致。清掉背景图后一切恢复原样，DSH 自己的主题设置重新说了算。
...
- 设置窗口有自己的 preload，**主窗口依旧不挂载任何 preload**，`sandbox` 与 `contextIsolation` 保持原样；外壳在主窗口里唯一的脚本求值是一段只读的调色板读取脚本。
- `scripts/preview-background.cjs` 把真实 DSH 页面渲染成 PNG，用来核对观感（用法见脚本头部注释）。

### 局限

- 「文件 → 在默认浏览器中打开」的那个页面**不会有背景**。它是独立运行的 `dsh web`，由系统浏览器渲染，桌面外壳无法向它注入样式。背景只作用于桌面窗口本身。
- 「界面配色」选自动/浅色/深色时，外壳把内置调色板以 `!important` 重新声明在 `body` 上，因此装了背景图期间，第三方主题（`ctx.theme` 注册的令牌覆盖）会被压住；选「跟随 DSH 主题」就交还给 DSH，第三方主题照常生效。
